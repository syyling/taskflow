...
export function ProjectDialog() {
    ...
    const handleSave = async () => {
        const { data, error } = await supabase
            .from('project')
            .insert([
                { "name": projectData.name, "description": projectData.description, "deadline": projectData.deadline, "progress": projectData.progress },
            ])
            .select();
        ...
        if (data) {
            ...
            projectData.selectedUsers.map(async (selectedUser) => {
                const {data: projectinusers, error} = await supabase
                    ...
                    .insert([
                        {"project_id": projectId, "user_id": selectedUser},
                    ])
                    .select();
            });
        }
                        ...
                        <Popover>
                            <PopoverTrigger asChild>
                                ...
                                >
                                    ...
                                    {projectData.deadline ? format(projectData.deadline, "yyyy-MM-dd") :
                        ...
                        <Select onValueChange={(value) => handleInputChange('progress', value)}>
                            ...
                            <SelectContent>
                                {progressOptions.map((progressOption) => (
                                    <SelectItem key={progressOption.id} value={progressOption.label}>
